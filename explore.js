...
function getEventDefaultPoster(eventId, title) {
  ...
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}`;
    ...
    // Poster element
    const effectivePoster = (ev.poster && ev.poster.trim()) ? ev.poster : getEventDefaultPoster(ev.id, ev.title);
    const safeTitle = (ev.title || "Event").replace(/'/g, "\\'");
    const posterHTML = `<img src="${effectivePoster}" class="event-card-poster" alt="${ev.title} poster" onclick="openPosterLightbox('${effectivePoster.replace(/'/g, "\\'")}', '${safeTitle}')" style="cursor: pointer;" title="Click to view full poster">`;
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 20px; align-items: start; margin-top: 15px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 14px;">
          ...
          <div class="event-details" style="margin: 0; grid-template-columns: 1fr 1fr; display: grid; gap: 10px; background: rgba(0, 243, 255, 0.05); border: 1px solid rgba(0, 243, 255, 0.3); border-radius: 10px; padding: 14px 16px; font-size: 0.85rem;">
            ...
            <div style="grid-column: 1/-1; color: ${isClosed ? 'var(--neon-red)' : 'var(--neon-green)'};">⏳ <strong>Reg Close:</strong> ${regCloseText}</div>
          </div>
